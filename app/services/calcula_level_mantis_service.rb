class CalculaLevelMantisService
  ...
  def execute
    ...
    # Novo ticket
    if @old_ticket['created_at'].to_s == @old_ticket['updated_at'].to_s && @new_ticket.status == 'resolvido'
      pontos += 1
    end
    pontos += 2 * @new_comments.size
    if @old_ticket['status'] == 'resolvido'
      pontos += 2
      case @new_ticket.priority
      when "imediato"
        pontos += condicional_type(@old_ticket['updated_at'].to_s.to_date, @new_ticket.created_at.to_s.to_date, "imediato")
      when "urgente"
        pontos += condicional_type(@old_ticket['updated_at'].to_s.to_date, @new_ticket.created_at.to_s.to_date, "urgente")
      when "alta"
        pontos += condicional_type(@old_ticket['updated_at'].to_s.to_date, @new_ticket.created_at.to_s.to_date, "alta")
      when "normal"
        pontos += condicional_type(@old_ticket['updated_at'].to_s.to_date, @new_ticket.created_at.to_s.to_date, "normal")
      when "baixa"
        pontos += condicional_type(@old_ticket['updated_at'].to_s.to_date, @new_ticket.created_at.to_s.to_date, "baixa")
      else
        pontos += 0
      end
